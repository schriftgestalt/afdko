...
private procedure psDIV() {
	...
	x = Pop();
	if (y == FixInt(100))
		x /= 100L; /* this will usually be the case */
	else
		x = (x * FixOne) / y;
	Push(x);
}
...
private procedure RDcurveto(c1, c2, c3) Cd c1, c2, c3; {
	if (!forMultiMaster) {
		...
		new->x1 = tfmx(ScaleAbs(c1.x));
		new->y1 = tfmy(ScaleAbs(c1.y));
		new->x2 = tfmx(ScaleAbs(c2.x));
		new->y2 = tfmy(ScaleAbs(c2.y));
		new->x3 = tfmx(ScaleAbs(c3.x));
		new->y3 = tfmy(ScaleAbs(c3.y));
	}
	else {
		PCharPathElt new;
...
private procedure RDmtlt(etype) integer etype; {
	if (!forMultiMaster) {
		...
		new->x = tfmx(ScaleAbs(currentx));
		new->y = tfmy(ScaleAbs(currenty));
	}
	else {
		PCharPathElt new;
...
private procedure Rct(Cd c1, Cd c2, Cd c3) {
	tempx = currentx;
	tempy = currenty;
	DoDelta(c1.x, c1.y);
	c1.x = currentx;
...
private procedure ReadHintInfo(nm, str) char nm;
const char *str; {
	Cd c0;
	short hinttype = nm == 'y' ? RY : nm == 'b' ? RB : nm == 'm' ? RM + ESCVAL : RV + ESCVAL;
	long elt1, elt2;
	PopPCd(&c0);
...
private procedure ParseString(s) const char *s; {
	...
	nxtChar:
		...
		case '%': /* comment */
			if (bezGlyphName[0] == 0) {
				...
				if (end < 64)
					...
					bezGlyphName[63] = 0;
					sprintf(globmsg, "Bad input file.  Glyph name  %s is greater than 32 chars.\n", bezGlyphName);
					LogMsg(globmsg, LOGERROR, NONFATALERROR, TRUE);
				}
			}
			while ((*s != '\n') && (*s != '\r')) {
		...
		case 0: /* end of file */
			if (stkindex != 0) {
				FlushLogFiles();
				sprintf(globmsg, "Bad input file.  Numbers left on stack at end of %s file.\n",
						fileName);
				LogMsg(globmsg, LOGERROR, NONFATALERROR, TRUE);
			}
			return;
	...
	rdnum:
		...
		while (TRUE) {
			...
			if (c == '.')
				...
				FlushLogFiles();
				sprintf(globmsg, "Illegal number terminator while reading %s file.\n", fileName);
				LogMsg(globmsg, LOGERROR, NONFATALERROR, TRUE);
				return;
			}
		} /*end while true */
...
public procedure SetReadFileName(file_name) char *file_name; {
	fileName = file_name;
}
...
public boolean ReadCharFile(normal, forBlendData, readHints, prependprefix)
...
#ifndef IS_LIB
	{
		...
		s = inputbuff;
		for (i = 0; i < cc; i++)
			*s++ = 0; /* erase the plain text version */
	}
#endif
#endif
#ifndef IS_LIB
