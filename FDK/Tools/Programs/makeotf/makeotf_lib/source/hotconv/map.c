...
#define DM(p)            do { if (DB_MAP(g)) { printf p; } \
} \
	while (0)
...
typedef struct {
	short bitNum;           /* in OS_2.CodePageRange1+2; -1 means not a Windows
	                           code page. */
...
static int mapName2UVOverrideName(hotCtx g, char *gname, char **uvOverrideName) {
	...
	uvName = (uvOverrideName != NULL && g->cb.getUVOverrideName != NULL)
	    ? g->cb.getUVOverrideName(g->cb.ctx, gname) : NULL;
...
static void CDECL CMapMsg(mapCtx h, int msgType, char *fmt, ...) {
	...
	va_end(ap);
	sprintf(msg, "%s [%s]", msgVar, h->ps.cb.psId ?
	        h->ps.cb.psId(h->ps.cb.ctx) : "");
...
#define IS_UNSET(var) ((var) != 0 && (var) != 1)
	...
	for (i = 0; i < nRanges; i++) {
		...
		if (!vert) {
			Range *range = codespace ? dnaNEXT(tmpCodespace) :
			    *isMac ? dnaNEXT(h->cid.mac.range) : dnaNEXT(h->cid.hor.range);
	...
	if (!psMatchToken(ps, psGetToken(ps), PS_OPERATOR,
	                  codespace ? "endcodespacerange" :
	                  notdef ? "endnotdefrange" : charrange ? "endcidchar" : "endcidrange")) {
...
void mapAddCMap(hotCtx g, hotCMapId id, hotCMapRefill refill) {
	...
	if (h->cid.hor.name != SINX_UNDEF && h->cid.ver.name != SINX_UNDEF) {
		...
		if (strcmp(STR(h->cid.ver.usecmap), STR(h->cid.hor.name)) != 0) {
			CMapMsg(h, hotFATAL, "usecmap resource <%s> in V Unicode CMap not "
			        "found", STR(h->cid.ver.usecmap));
		}
	}
}
			...
			if (uc != NULL &&
				...
				if (g->font.flags & HOT_DOUBLE_MAP_GLYPHS) {
					...
					if ((uc = getUVFromAGL(g, dblMap, 0)) != NULL) {
						...
						MAKE_UNI_GLYPH(uc->uv, uni);
						if (mapName2Glyph(g, uni, NULL) == NULL) {
							/* Map glyph at secondary UV also. No uni<CODE>
							   override is present */
							addUVToGlyph(g, gi, uc->uv);
							assignedUVs++;
						}
				...
				if (assignedUVs == 0) {
					...
					hotMsg(g, hotWARNING, "glyph <%s> not encoded in Unicode "
				...
				/* Need to identify whether it's a base or ligature of a supp
				   UV, for proper EUS sorting */
...
static void setOS_2Fields(hotCtx g) {
	...
	/*
	   Xcode compiler kept complaing about signed/unsigned mistmach - can't figure out why.
 ...
 */
	...
	/* rroberts Nov 21 2002. I am re-enabling this, as it turns out that a font with only the OEM code page set
	   is not well supported by Windows 2000 CharMap. This happens to GamePiLTStd-ChessDraughts.otf,
	...
	/* If the mac bit is set make sure that the LATIN_BIT is set as well. Otherwise, for a few fonts with just
	   the basic latin set, like LucidaMathStd-italic, only the Mac bit gets set, and this seems to be
	   ignored by Wdinows, so you can't type any test. with it.*/
...
static void createMaccmap(hotCtx g, long script, long language,
	...
	if (g->font.mac.cmapScript == HOT_CMAP_UNKNOWN) {
		if (script != HOT_CMAP_UNKNOWN) {
			...
			g->font.mac.cmapScript = (macCPInx >= 0) ?
			    codePage[macCPInx].maccmapScript : MAC_SCR_ROMAN;
		}
	}
	...
	if (g->font.mac.cmapLanguage == HOT_CMAP_UNKNOWN) {
		if (language != HOT_CMAP_UNKNOWN) {
			...
			g->font.mac.cmapLanguage = (macCPInx >= 0) ?
			    codePage[macCPInx].maccmapLanguage : MAC_LANG_UNSPEC;
		}
	}
...
static void makeCIDMaccmap(hotCtx g) {
	...
	if (g->font.mac.cmapScript == HOT_CMAP_UNKNOWN) {
		hotMsg(g, hotWARNING, "can't autodetect Macintosh cmap script for CID "
		       "font <%s>; defaulting to Roman script", g->font.FontName.array);
...
static void makeUVScmap(hotCtx g) {
	...
	for (i = 0; i < h->uvs.entries.cnt; i++) {
		...
		if (IS_CID(g)) {
			...
			gi_id = mapName2Glyph(g, uve->gName, NULL);
		}
...
static GID kernGetGID(hotCtx g, unsigned index) {
	if (index & KERN_UNENC) {
		...
		return (iChar >= g->font.unenc.cnt) ? GID_UNDEF :
		       mapName2GID(g, g->font.unenc.array[iChar].array, NULL);
	}
	else {
		/* Index is char code */
...
static void AFMPrintCharMetrics(hotCtx g) {
	...
	if (IS_CID(g)) {
		...
		/* Store and sort AFM chars in canonical order. Caution: multiply
		   encoded glyphs */
		...
		for (i = 0; i < 256; i++) {
			...
			if (gname != NULL) {
				h->sort.platEnc[i] = (strcmp(gname, ".notdef") == 0)
				    ? NULL : mapName2Glyph(g, gname, NULL);
			}
		}
	}
